...
type NavigationProp = NativeStackNavigationProp<TeacherTrainingPlanStackParamList, 'TeacherTrainingPlanDetails'>;

export const TeacherTrainingPlanDetails = () => {
    ...
    if (isEditing) {
      ...
      if (plan) {
        if (plan.createdBy !== user?.uid) {
           Alert.alert('Unauthorized', 'You do not have permission to edit this training plan.');
           navigation.goBack();
           return;
        }
        setName(plan.name);
  ...
  }, [dispatch, isEditing, planId, trainingPlans, user, navigation]);
  ...
  const handlePickImage = async () => {
    ...
    if (result.assets[0].fileSize && result.assets[0].fileSize > VALIDATION_LIMITS.IMAGE.MAX_SIZE_BYTES) {
      Alert.alert(t('adminTrainingPlanDetails.uploadError') || 'Error', `Image must be under ${VALIDATION_LIMITS.IMAGE.MAX_SIZE_MB}MB`);
      return;
    }
    ...
    try {
      ...
      Alert.alert(t('adminTrainingPlanDetails.uploadFailed') || 'Upload Failed', error.message || t('adminTrainingPlanDetails.uploadError'));
    } finally {
  ...
  const handleSave = () => {
    ...
    if (!name.trim()) {
      ...
      errors.name = t('adminTrainingPlanDetails.nameMinLength', { length: VALIDATION_LIMITS.TRAINING_PLAN.NAME_MIN_LENGTH }) || `Name must be at least ${VALIDATION_LIMITS.TRAINING_PLAN.NAME_MIN_LENGTH} characters.`;
      hasError = true;
    }
    ...
    if (!description.trim()) {
      ...
      errors.description = t('adminTrainingPlanDetails.descMinLength', { length: VALIDATION_LIMITS.TRAINING_PLAN.DESCRIPTION_MIN_LENGTH }) || `Description must be at least ${VALIDATION_LIMITS.TRAINING_PLAN.DESCRIPTION_MIN_LENGTH} characters.`;
      hasError = true;
    }
    ...
    if (isUploading) {
      Alert.alert(t('adminTrainingPlanDetails.pleaseWait') || 'Please wait', t('adminTrainingPlanDetails.uploading') || 'Uploading image...');
      return;
    }
    ...
    const payload = {
      ...
      createdBy: user?.uid, // specific to teacher
    };
  ...
  const availableCourses = courses.filter(c => 
    c.createdBy === user?.uid && // Teacher only sees their courses
  ...
  const renderHeader = () => (
    <View style={[styles.header, { paddingTop: insets.top || SPACING.md }]}>
      ...
      <Text style={styles.headerTitle}>{t('adminTrainingPlanDetails.planManager') || 'Plan Manager'}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {isEditing && <Text style={styles.modeOverline}>{t('adminTrainingPlanDetails.editingMode') || 'EDITING MODE'}</Text>}
        <Text style={TYPOGRAPHY.headline}>{isEditing ? (t('adminTrainingPlanDetails.planDetails') || 'Plan Details') : (t('adminTrainingPlanDetails.createNewPlan') || 'Create New Plan')}</Text>

        <View style={styles.formGroup}>
          <Text style={styles.label}>{t('adminTrainingPlanDetails.planName') || 'Plan Name'}</Text>
          <TextInput
            style={[styles.input, formErrors.name ? styles.inputError : null]}
            placeholder={t('adminTrainingPlanDetails.enterPlanName') || 'Enter plan name'}
        ...
        >
          {isUploading ? (
            ...
            <View style={styles.thumbnailPlaceholder}>
              ...
              <Text style={styles.thumbUploadText}>{isEditing ? (t('adminTrainingPlanDetails.changeCover') || 'Change Cover') : (t('adminTrainingPlanDetails.uploadCover') || 'Upload Cover')}</Text>
            </View>
          )}
        </TouchableOpacity>
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>{t('adminTrainingPlanDetails.description') || 'Description'}</Text>
          <TextInput
            style={[styles.input, styles.textArea, formErrors.description ? styles.inputError : null]}
            placeholder={t('adminTrainingPlanDetails.enterDescription') || 'Enter description'}
            placeholderTextColor={COLORS.outline}
            multiline
            numberOfLines={4}
            value={description}
        ...
        <View style={styles.visibilityCard}>
          <View style={styles.visibilityInfo}>
            ...
            <View>
              <Text style={styles.visibilityTitle}>{t('adminTrainingPlanDetails.publicVisibility') || 'Public Visibility'}</Text>
              <Text style={styles.visibilityDesc}>{t('adminTrainingPlanDetails.publicVisibilityDesc') || 'Anyone can view'}</Text>
            </View>
          </View>
          <Switch 
            value={publicAccess}
        ...
        <View style={styles.sectionHeaderRow}>
          <Text style={styles.sectionTitle}>{t('adminTrainingPlanDetails.curriculum') || 'Curriculum'}</Text>
          <TouchableOpacity style={styles.addCourseBtn} onPress={() => setShowCourseModal(true)}>
            <Text style={styles.addCourseText}>{t('adminTrainingPlanDetails.addCourse') || 'Add Course'}</Text>
          </TouchableOpacity>
        </View>
        ...
        {curriculumCourses.map((course, index) => (
          <View key={course.id} style={styles.lessonCard}>
            ...
            <View style={styles.lessonIconWrapper}>
                <BookOpen size={18} color={COLORS.onSurface} />
            </View>
        ...
        {curriculumCourses.length === 0 && (
          <View style={{ alignItems: 'center', marginVertical: SPACING.xl }}>
             <Text style={{ color: COLORS.outline }}>{t('adminTrainingPlanDetails.noCoursesAdded') || 'No courses added yet'}</Text>
          </View>
        )}
      ...
      <View style={[styles.footer, { paddingVertical: 10, paddingBottom: 10 }]}>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.saveCourseText}>{isEditing ? (t('adminTrainingPlanDetails.updatePlan') || 'Update Plan') : (t('adminTrainingPlanDetails.createPlan') || 'Create Plan')}</Text>
          )}
        </TouchableOpacity>
      </View>
      ...
      <Modal visible={showCourseModal} animationType="slide" presentationStyle="formSheet">
        <View style={styles.modalBg}>
            <View style={styles.modalHeader}>
              ...
              <Text style={styles.headerTitle}>{t('adminTrainingPlanDetails.selectCourses') || 'Select Courses'}</Text>
              <View style={{ width: 32 }} />
            </View>
            ...
            <View style={styles.modalSearch}>
              ...
              <TextInput
                ...
                placeholder={t('adminTrainingPlanDetails.searchCourses') || 'Search courses...'}
                placeholderTextColor={COLORS.outline}
                value={courseSearch}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={styles.modalLessonCard}>
                  ...
                  >
                    <Text style={styles.actionIconPrimary}>{t('adminTrainingPlanDetails.add') || 'ADD'}</Text>
                  </TouchableOpacity>
                </View>
              )}
              ListEmptyComponent={
                 <View style={{ alignItems: 'center', marginTop: 40 }}>
                    <Text style={{ color: COLORS.outline }}>{t('adminTrainingPlanDetails.noMatchingCourses') || 'No matching courses found'}</Text>
                 </View>
              }
            />
            ...
            <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, SPACING.md) }]}>
               <TouchableOpacity style={styles.saveCourseBtn} onPress={() => setShowCourseModal(false)}>
                 <Text style={styles.saveCourseText}>{t('adminTrainingPlanDetails.done') || 'Done'}</Text>
               </TouchableOpacity>
            </View>
        </View>
      </Modal>
