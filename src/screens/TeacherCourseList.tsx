...
type NavigationProp = NativeStackNavigationProp<TeacherCourseStackParamList, 'TeacherCourseList'>;
...
const TeacherCourseList = () => {
  ...
  const handleDelete = (id: string, title: string) => {
    Alert.alert(
      t('adminCourses.deleteTitle') || 'Delete Course',
      (t('adminCourses.deleteConfirm', { title }) || `Are you sure you want to delete ${title}?`),
      [
        { text: t('adminCourses.cancel') || 'Cancel', style: 'cancel' },
        { 
          text: t('adminCourses.delete') || 'Delete', 
          style: 'destructive', 
      ...
      <View style={[styles.cardContainer, isGrid ? styles.cardGrid : styles.cardList]}>
        <View style={[styles.imageContainer, isGrid ? { height: 120 } : { height: 160 }]}>
          ...
          <View style={[styles.badge, isPublic ? styles.badgePublic : styles.badgePrivate]}>
            ...
            <Text style={[styles.badgeText, isPublic ? styles.badgeTextPublic : styles.badgeTextPrivate]}>
              {isPublic ? (t('adminCourses.publicBadge') || 'PUBLIC') : (t('adminCourses.privateBadge') || 'PRIVATE')}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.cardContent}>
          ...
          <View style={styles.instructorRow}>
            ...
            <Text style={styles.instructorName} numberOfLines={1}>{item.instructor || (t('adminCourses.unknown') || 'Unknown')}</Text>
          </View>
          
          <View style={styles.statsRow}>
            <Users size={12} color={COLORS.outline} />
            <Text style={styles.learnersText}>{(t('adminCourses.learnersCount', { count: learnerCount.toLocaleString() }) || `${learnerCount} learners`)}</Text>
          </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top || SPACING.md }]}>
        <Text style={TYPOGRAPHY.headline}>{t('adminCourses.manageCourses') || 'My Courses'}</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => navigation.navigate('TeacherCourseDetails', {})}
          >
      ...
      {isSearchVisible && (
        <View style={styles.searchContainer}>
          ...
          <TextInput 
            ...
            placeholder={t('adminCourses.searchPlaceholder') || 'Search courses...'}
      ...
      <View style={styles.controlsRow}>
        ...
        <View style={styles.filtersWrapper}>
          ...
          >
            <Text style={[styles.filterText, filter === 'all' && styles.filterTextActive]}>{t('adminCourses.all') || 'All'}</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[styles.filterChip, filter === 'public' && styles.filterChipActive]}
            onPress={() => setFilter('public')}
          >
            <Text style={[styles.filterText, filter === 'public' && styles.filterTextActive]}>{t('adminCourses.public') || 'Public'}</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[styles.filterChip, filter === 'private' && styles.filterChipActive]}
            onPress={() => setFilter('private')}
          >
            <Text style={[styles.filterText, filter === 'private' && styles.filterTextActive]}>{t('adminCourses.private') || 'Private'}</Text>
          </TouchableOpacity>
        </View>
      </View>
